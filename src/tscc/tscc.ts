...
class TSCC {
  ...
  constructor(grammar: Grammar, argument: TSCCParameter) {
    ...
    grammar.association?.forEach((v, i, a) => {
      if (Object.getOwnPropertyNames(v).length != 1) {
        ...
        if (keys.length == 1) {
          ...
          if (/^\s*[^"`:\s$@#]+\s*:\s*([^\s`"$@#]+\s*)*$/.test(keys[0])) {
            ...
            while ((bodySym = bodyReg.exec(keys[0])) != null) {
              ...
              if (this.TerminalSymbols.has(bodySym[1])) {
                //在终结符表中尝试取得当前符号的结合性描述
                description = this.TerminalSymbols.get(bodySym[1]);
              }
              if (bodySym[1] == `error`) {
                //如果当前符号为'error',则将'error'添加到终结符表中
            ...
            if (!this.NTSSyntax.has(syntax[0])) {
              //记录该非终结符可以推导出的产生式
  ...
  private nullable(str: string[], expands: Set<string>): boolean {
    for (let i = 0; i < str.length; i++) {
      if (this.TerminalSymbols.has(str[i])) {
        ...
        if (!expands.has(str[i])) {
          //如果这个非终结符没有被展开过,则找到这个非终结符能推导的串
...
export default function Parse(lexer: Lex,Context?:any):any {
    let state: { [key: string]: string | undefined }[] = ${JSON.stringify(gotoTable)} as const;
    ...
    for (let i = 0; i < this.syntaxs.length; i++) {
      ...
      if (this.syntaxs[i].resolver != undefined) {
        //如果函数不是未定义,则输出到源码
...
`;
  ...
  private itemsFamily(startItem: Item): {}[] | undefined {
    ...
    L0: for (let i = 0; i < state.length; i++) {
      //遍历所有项集,处理移入规约操作,暂时不管移入-规约冲突和规约-规约冲突,先报错
      ...
      for (let j = 0; j < nowState.length; j++) {
        ...
        if (nowitem.dot < this.syntaxs[nowitem.syntax].length) {
          //点没有在最后一个位置,本次移入
          ...
          if (actionItem.has(followSymbol) && (actionItem.get(followSymbol)?.actionType == ActionType.reduce || actionItem.get(followSymbol)?.actionType == ActionType.err)) {
              ...
              if (syntaxPad.priority > symbolPriority) {
                ...
                if (syntaxPad.association == 'left') {
                  ...
                  actionItem.set(followSymbol, {
                    ...
                    target: 0,
                    shiftPriory: syntaxPad.priority,
                  }); //移入可以使用target:0,后面会自动填充该值
                } else {
                  //无结合性的
                  actionItem.set(followSymbol, {
                    actionType: ActionType.err,
                    target: 0,
                    ReducePad: syntaxPad,
                  }); //将动作设置为err，优先级和结合性保持和产生式一致
                }
              } else {
                //产生式的优先级比符号小
                //选择移入
                actionItem.set(followSymbol, {
                  actionType: ActionType.shift,
                  target: 0,
                  shiftPriory: symbolPriority,
                }); //移入可以使用target:0,后面会自动填充该值
              }
            } else if (syntaxPad == undefined && symbolPriority != undefined) {
              //产生式没有pad,符号有,E:E + E < E,如果<没有定义pad,则该产生式也没有pad
              //选择移入
              actionItem.set(followSymbol, {
                actionType: ActionType.shift,
                target: 0,
            ...
            actionItem.set(followSymbol, {
              actionType: ActionType.shift,
              target: 0,
              shiftPriory: this.TerminalSymbols.get(followSymbol)?.priority,
            }); //移入可以使用target:0,后面会自动填充该值
          }
        } else {
          //本次规约
          ...
          if (actionItem.has(followSymbol) && (actionItem.get(followSymbol)?.actionType == ActionType.reduce || actionItem.get(followSymbol)?.actionType == ActionType.err)) {
            ...
            if (lastPriority < nowPriority) {
              //选择优先级高的产生式进行规约,如果当前优先级大于之前项对应产生式的优先级则更改ActionItme，否则不需要变更
              actionItem.set(followSymbol, {
                actionType: ActionType.reduce,
                target: nowitem.syntax,
                ReducePad: syntaxPad,
              }); //指明规约产生式
            } else if (lastPriority == nowPriority) {
              //产生式优先级一致
              if (actionItem.get(followSymbol)!.target > nowitem.syntax) {
                //选择序号较小的产生式进行规约
                actionItem.set(followSymbol, {
                  actionType: ActionType.reduce,
                  target: nowitem.syntax,
            ...
            actionItem.set(followSymbol, {
              actionType: ActionType.reduce,
              target: nowitem.syntax,
              ReducePad: this.syntaxs[nowitem.syntax].pad,
            }); //指明规约产生式
          }
        }
      }
      if (tipMessageInThisState) {
        //当前状态处理中需要提示信息
    ...
    for (let i = 0; i < rules_use.length; i++) {
      //检查没有被归约过的产生式
  ...
  private closure(items: Item[]) {
    ...
    for (let i = 0; i < items.length; i++) {
      let item = items[i];
      if (item.dot < this.syntaxs[item.syntax].length) {
        //点没有在最后一个位置
        let followSymbol = this.syntaxs[item.syntax][item.dot]; //取得点后面的符号
        if (!this.TerminalSymbols.has(followSymbol)) {
          //点后面符号不是终结符则展开
          if (this.NTSSyntax.has(followSymbol)) {
            ...
            for (let j = 0; j < firstSet.length; j++) {
              for (let k = 0; k < syntaxs.length; k++) {
                ...
                if (!set.has(this.item2string(item_new))) {
                  //如果项集中没有该项,则添加到项集中
                  items.push(item_new);
  ...
  private goto(items: Item[], symbol: string): Item[] {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (dot < this.syntaxs[syntax].length) {
        //点后面还有符号
        if (this.syntaxs[syntax][dot] == symbol) {
          //如果点后面的符号和symbol一样,则将其添加到目标项集中
